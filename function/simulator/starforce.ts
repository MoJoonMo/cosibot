...
async function makePrice(msg:string){
...
async function calcStarforce(msg:string) {
    let m = msg.split(" ");
    ...
    let tv = await starforceExpectedValue(level + " " + event + " " + starCatch + " " + mvpDiscount); // 레벨, 이벤트, 스타캐치, MVP 할인

    return await makePrice("" + (tv[to] - tv[from]));
}

async function starforceExpectedValue(msg:string) {
    let m = msg.split(" ");
    let level = parseInt(m[0]);
    let event = m[1];
    let starCatch = m[2];
    ...
    for (let i = 0 ; i <= 25; i++) {
        /* 스타포스 비용 구하기 */
        c[i] = await starforceCost(level + " " + i + m[1] +" preventDestroy") * discountRate; // 30% 할인 여부
...
//        let successProbabilityBefore = sp[starforce-1];
        ...
        ev[starforce] = Math.round ( (cost + 
            ...
            (destroyProbability + downProbability * destroyProbabilityBefore) * (0 - totalValue12 + totalValueBefore )
            )/successProbability 
        );
...
async function starforceCost(msg:string) { // 1회 강화비용
    let m = msg.split(" ");
...
async function starforceSuccessProbability(msg:string) { // 성공 확률
    let m = msg.split(" ");
    //m[0] = Starforcelevel
...
async function starforceDestroyProbability(msg:string){ // 파괴 확률
    let m = msg.split(" ");
    //m[0] = Starforcelevel
    let starforce = parseInt(m[0]);
...
export {calcStarforce, makePrice}
